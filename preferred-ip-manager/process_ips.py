...
内置从 Telegram 频道拉取最新 IP、深层大带宽挖掘（透传 -sl）、自适应双阶段保底（Auto-Fallback）、
丢包率过滤（-tlr）、端口按需过滤、敏感凭据脱敏以及测速后默认自动推送至 Cloudflare Workers 订阅端。
...
# --- 默认全局配置与路径查找 ---
DEFAULT_SPEEDTEST_URL = "https://movies.jackyang.cc.cd/download?size=200"
...
class IPSourceManager:
    ...
    def collect_ips(cls, skip_tg: bool = False, input_file: Optional[str] = None, allowed_ports: Optional[List[str]] = None, auto_yes: bool = False, proxy: str = "") -> Dict[str, List[Tuple[str, str]]]:
        ...
        if input_file:
            ...
            if not latest_file:
                log_warn("未找到从 Telegram 下载的文件，将仅使用在线订阅源及历史源。")
            else:
                log_info(f"成功识别到 Telegram 原始节点文件: {latest_file}")

                # 仅在交互式终端且未开启 auto_yes 时提示用户
                if not auto_yes and sys.stdin.isatty():
                    print("\n" + "=" * 65)
                    print("📢 提示: 从 Telegram 下载 IP 列表已完成！")
                    print("⚡ 请确认已【断开/关闭】系统全局代理，以确保后续直连测速准确。")
                    print("=" * 65)
                    try:
                        input("👉 请按回车键 (Enter) 开始测速: ")
        ...
        if input_file:
            ...
            log_info(f"汇总 IP 数据池完成: TG 下载源 ({file_ip_count} 个) + 现有订阅 IP (新增 {sub_added} 个) + 历史 IP ({history_added} 个)，共计 {total_ips} 个候选 IP 准备测速")

        return filtered_groups


# ==============================================================================
# 4. CloudflareSpeedTest 测速与双阶段自适应保底引擎 (CFSTRunner)
# ==============================================================================
...
class CFSTRunner:
    ...
    def execute_cfst(cls, temp_ip_path: str, temp_csv_path: str, port: str, min_speed: float, args: argparse.Namespace) -> List[Dict[str, Any]]:
        ...
        url_flag = f' -url "{args.speedtest_url}"' if args.speedtest_url else f' -url "{DEFAULT_SPEEDTEST_URL}"'
        ...
        log_info(f"参数解析明细: [目标端口: {port}] [测速模式: {args.mode}] [并发线程数: {args.concurrency}] [达标队列数: {args.test_count}] [单点时长: {args.download_time}s] [速度下限: {min_speed} MB/s] [延迟上限: {args.max_delay if args.max_delay else '不限制'} ms] [丢包上限: {f'{args.max_loss*100:.0f}%' if args.max_loss < 1.0 else '不限制'}] [测速URL: {args.speedtest_url}]")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    cdn_group.add_argument("--test-count", "-dn", type=int, default=20,
                           help="[带宽模式] 下载测速达标数量")
    cdn_group.add_argument("--url", "--speedtest-url", dest="speedtest_url",
                           default=os.getenv("CFST_URL", DEFAULT_SPEEDTEST_URL),
    ...
    parser.add_argument("--yes", "-y", action="store_true", help="非交互模式，自动推送结果至 Cloudflare Workers 订阅端")
    parser.add_argument("--no-upload", "--no-push", "--local-only", dest="no_upload", action="store_true",
                        help="仅在本地执行测速与保存文件，不推送至远程 Cloudflare Workers 订阅端")
...
def run_warp_workflow(args: argparse.Namespace):
    ...
    # 默认推送至 Workers 订阅端 (除非指定 --no-upload)
    ...
    if not token:
        ...
        if not args.yes and sys.stdin.isatty():
            try:
                user_input = input(f"\n👉 是否确认将以上 {len(top_results)} 个 WARP 优选端点推送到 Cloudflare Workers 订阅服务器？ [Y/n] (默认回车推送): ").strip().lower()
...
def run_cdn_workflow(args: argparse.Namespace):
    ...
    if top_results:
        ...
        if not token:
            ...
            if not args.yes and sys.stdin.isatty():
                try:
                    user_input = input(f"\n👉 是否确认将以上 {len(top_results)} 个优选 IP 推送到 Cloudflare Workers 订阅服务器？ [Y/n] (默认回车推送): ").strip().lower()
                    if user_input not in ('', 'y', 'yes'):
