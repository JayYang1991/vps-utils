...
check_if_running_as_root() {
  if [[ "$UID" -ne '0' ]]; then
    echo "WARNING: The user currently executing this script is not root. You may encounter insufficient privilege error."
    read -r -p "Are you sure you want to continue? [y/n] " cont_without_been_root
    if [[ x"${cont_without_been_root:0:1}" = x'y' ]]; then
      echo "Continuing the installation with the current user..."
    else
...
start_v2ray() {
  if [[ -f '/etc/systemd/system/v2ray.service' ]]; then
    if systemctl start "${V2RAY_CUSTOMIZE:-v2ray}"; then
      ...
      echo 'error: Failed to start the V2Ray service.'
      exit 1
    fi
  fi
}
...
show_help() {
  echo "usage: $0 [OPTIONS] [--mode MODE]"
  echo ''
  echo 'Installation Modes:'
  echo '  --mode standard      Standard V2Ray installation (default)'
  ...
  echo '  -f, --force       Force the installation of the latest version of V2Ray'
