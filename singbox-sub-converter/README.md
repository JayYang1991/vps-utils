...
4. **服务端规则持久化与离线/在线 API 转换、Sniffer 补丁与空测速组清理**:
   ...
   - **智能清理空/单 DIRECT 测速组**：生成 Clash 配置后，自动递归识别并清除仅包含 `DIRECT` / 无有效节点的 `url-test` 测速策略组，级联剔除上级分组中的失效引用，并将分流规则（`rules`）中的目标安全回退至主选择组，避免节点缺失时客户端界面残留大量冗余测速组。
